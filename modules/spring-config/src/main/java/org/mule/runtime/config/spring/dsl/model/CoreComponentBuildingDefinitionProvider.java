  ...
  private TypeConverter<String, ErrorTypeMatcher> getErrorTypeConverter() {
    return (value) -> {
      ...
      List<ErrorTypeMatcher> matchers = stream(errorTypeIdentifiers).map((identifier) -> {
        ...
        Optional<ErrorType> optional = muleContext.getErrorTypeRepository().lookupErrorType(parseComponentIdentifier(parsedIdentifier));
        ErrorType errorType = optional.orElseThrow(() -> new MuleRuntimeException(createStaticMessage("Could not found ErrorType for the given identifier: '%s'",parsedIdentifier)));
